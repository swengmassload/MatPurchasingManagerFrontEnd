...
import AllUsersForReport from "./AllUsersForReport";
import AllStagesDropDownForReport from "./AllStagesDropDownForReport";
import DateRangeComponentForReport from "./DateRangeComponentForReport";
import { SimpleBoxborder } from "../../../../../Components/Common/SimpleBoxborder";
import ProductIdNumberRange from "./ProductIdNumberRange";

import ContactNameForReport from "./ContactNameForReport";
import CompanyNameForReport from "./CompanyNameForReport";
import CustomerEmailForReport from "./CustomerEmailForReport";
import SalesOrderIdForReport from "./SalesOrderIdForReport";

interface RMAReportSpecifierProps {
...
const RMAReportSpecifier: React.FC<RMAReportSpecifierProps> = ({
  startDateIssued,
  endDateIssued,
  setStartDateIssued,
  setEndDateIssued,
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%", alignItems: "flex-start" }}>
          ...
          >
            ...
            <Box sx={{ ...SimpleBoxborder, flexDirection: "column", gap: 1, alignItems: "center", width: "100%" }}>
              ...
              <Box sx={{ display: "flex", gap: 2, p: 1, alignItems: "center", width: "100%" }}>
                <CompanyNameForReport companyName={companyName} setCompanyName={setCompanyName} />
                <ContactNameForReport contactName={contactName} setContactName={setContactName} />
              </Box>
            </Box>
          </Box>
        </Box>
      ...
      >
        <Box sx={{ ...SimpleBoxborder, width: "100%", gap: "2rem", padding: "1rem" }}>
          <CustomerEmailForReport customerEmail={customerEmail} setCustomerEmail={setCustomerEmail} />
          <SalesOrderIdForReport salesOrderId={salesOrderId} setSalesOrderId={setSalesOrderId} />
        </Box>
      </Box>
      ...
      >
        <Box sx={{ ...SimpleBoxborder, width: "100%", gap: "2rem", padding: "1rem" }}>
          <AllStagesDropDownForReport stage={stage} setStage={setStage} />
          <AllUsersForReport salesPerson={salesPerson} setSalesPerson={setSalesPerson} />
        </Box>
      </Box>
      {/* Generate Button */}
      {/* <Box sx={{ display: "flex", gap: "1rem", width: "100%", justifyContent: "center", padding: "1rem" }}>
...
export default  RMAReportSpecifier;
