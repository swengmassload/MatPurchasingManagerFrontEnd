...
export const useCreateModel = () => {
  ...
  return useMutation({
    mutationFn: (data: ModelCreateRequestDTO) =>
      CRUDApi<ModelCreateRequestDTO, ModelCreateRequestDTO>(BASEAPIURL + ModelManagerEnpoints.ModelEndpoint).addData(
        data
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QueryKeys.useGetModelKey.mainKey] });
    },
  });
};
